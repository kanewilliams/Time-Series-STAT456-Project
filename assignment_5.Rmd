...
This data will then be reduced to the 10 weeks beginning from `5 December 2022`. This date and length was chosen because it (i) includes the Christmas period, which will be a good test for how it deals with this, and (ii) conveniently begins on a Monday which will make the plots easier to interpret. 
...
ggplot(elec_data, aes(x = Period_Start, y = Price)) +
  geom_line() +
  geom_vline(xintercept = as.POSIXct("2022-12-25"), color = "red", linetype = "dashed", size = 1) +
...
# only one seasonality allowed, so weekly chosen
